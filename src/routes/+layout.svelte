...
<div class="flex min-h-screen flex-col">
	<section class="flex flex-1">
				...
				{#if item.children}
					{#each item.children as child}
						<Button
							...
							href="{base}/{child.href}">{child.name}</Button
						>
					{/each}
				{/if}
			{/each}
		...
		<div class="h-screen flex-1 overflow-y-auto">
			<Navbar>
				<Button variant="text" size="icon" aria-label="drawer" onclick={() => (drawer = !drawer)}>
					<MenuIcon />
				</Button>
				<div class="flex-1"></div>
				<Button variant="text" aria-label="components" href={base}>Components</Button>
				<Button
					variant="text"
					size="icon"
					aria-label="github"
